...
public abstract class NavigationBarView extends FrameLayout {
    ...
    public NavigationBarView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        TintTypedArray attributes = ThemeEnforcement.obtainTintedStyledAttributes(context2, attrs, C0489R.styleable.NavigationBarView, defStyleAttr, defStyleRes, C0489R.styleable.NavigationBarView_itemTextAppearanceInactive, C0489R.styleable.NavigationBarView_itemTextAppearanceActive);
        ...
        boolean hasValue = attributes.hasValue(C0489R.styleable.NavigationBarView_itemIconTint);
        NavigationBarMenuView navigationBarMenuView = this.menuView;
        if (hasValue) {
            navigationBarMenuView.setIconTintList(attributes.getColorStateList(C0489R.styleable.NavigationBarView_itemIconTint));
        } else {
            navigationBarMenuView.setIconTintList(this.menuView.createDefaultColorStateList(R.attr.textColorSecondary));
        }
        setItemIconSize(attributes.getDimensionPixelSize(C0489R.styleable.NavigationBarView_itemIconSize, getResources().getDimensionPixelSize(C0489R.dimen.mtrl_navigation_bar_item_default_icon_size)));
        if (attributes.hasValue(C0489R.styleable.NavigationBarView_itemTextAppearanceInactive)) {
            setItemTextAppearanceInactive(attributes.getResourceId(C0489R.styleable.NavigationBarView_itemTextAppearanceInactive, 0));
        }
        if (attributes.hasValue(C0489R.styleable.NavigationBarView_itemTextAppearanceActive)) {
            setItemTextAppearanceActive(attributes.getResourceId(C0489R.styleable.NavigationBarView_itemTextAppearanceActive, 0));
        }
        boolean isBold = attributes.getBoolean(C0489R.styleable.NavigationBarView_itemTextAppearanceActiveBoldEnabled, true);
        setItemTextAppearanceActiveBoldEnabled(isBold);
        if (attributes.hasValue(C0489R.styleable.NavigationBarView_itemTextColor)) {
            setItemTextColor(attributes.getColorStateList(C0489R.styleable.NavigationBarView_itemTextColor));
        }
        ...
        if (attributes.hasValue(C0489R.styleable.NavigationBarView_itemPaddingTop)) {
            setItemPaddingTop(attributes.getDimensionPixelSize(C0489R.styleable.NavigationBarView_itemPaddingTop, 0));
        }
        if (attributes.hasValue(C0489R.styleable.NavigationBarView_itemPaddingBottom)) {
            setItemPaddingBottom(attributes.getDimensionPixelSize(C0489R.styleable.NavigationBarView_itemPaddingBottom, 0));
        }
        if (attributes.hasValue(C0489R.styleable.NavigationBarView_activeIndicatorLabelPadding)) {
            setActiveIndicatorLabelPadding(attributes.getDimensionPixelSize(C0489R.styleable.NavigationBarView_activeIndicatorLabelPadding, 0));
        }
        if (attributes.hasValue(C0489R.styleable.NavigationBarView_elevation)) {
            setElevation(attributes.getDimensionPixelSize(C0489R.styleable.NavigationBarView_elevation, 0));
        }
        ColorStateList backgroundTint = MaterialResources.getColorStateList(context2, attributes, C0489R.styleable.NavigationBarView_backgroundTint);
        DrawableCompat.setTintList(getBackground().mutate(), backgroundTint);
        setLabelVisibilityMode(attributes.getInteger(C0489R.styleable.NavigationBarView_labelVisibilityMode, -1));
        int itemBackground = attributes.getResourceId(C0489R.styleable.NavigationBarView_itemBackground, 0);
        if (itemBackground != 0) {
            this.menuView.setItemBackgroundRes(itemBackground);
        } else {
            setItemRippleColor(MaterialResources.getColorStateList(context2, attributes, C0489R.styleable.NavigationBarView_itemRippleColor));
        }
        int activeIndicatorStyleResId = attributes.getResourceId(C0489R.styleable.NavigationBarView_itemActiveIndicatorStyle, 0);
        if (activeIndicatorStyleResId != 0) {
            setItemActiveIndicatorEnabled(true);
            TypedArray activeIndicatorAttributes = context2.obtainStyledAttributes(activeIndicatorStyleResId, C0489R.styleable.NavigationBarActiveIndicator);
            int itemActiveIndicatorWidth = activeIndicatorAttributes.getDimensionPixelSize(C0489R.styleable.NavigationBarActiveIndicator_android_width, 0);
            setItemActiveIndicatorWidth(itemActiveIndicatorWidth);
            int itemActiveIndicatorHeight = activeIndicatorAttributes.getDimensionPixelSize(C0489R.styleable.NavigationBarActiveIndicator_android_height, 0);
            setItemActiveIndicatorHeight(itemActiveIndicatorHeight);
            int itemActiveIndicatorMarginHorizontal = activeIndicatorAttributes.getDimensionPixelOffset(C0489R.styleable.NavigationBarActiveIndicator_marginHorizontal, 0);
            setItemActiveIndicatorMarginHorizontal(itemActiveIndicatorMarginHorizontal);
            ColorStateList itemActiveIndicatorColor = MaterialResources.getColorStateList(context2, activeIndicatorAttributes, C0489R.styleable.NavigationBarActiveIndicator_android_color);
            setItemActiveIndicatorColor(itemActiveIndicatorColor);
            int shapeAppearanceResId = activeIndicatorAttributes.getResourceId(C0489R.styleable.NavigationBarActiveIndicator_shapeAppearance, 0);
        ...
        int shapeAppearanceResId2 = C0489R.styleable.NavigationBarView_menu;
        if (attributes.hasValue(shapeAppearanceResId2)) {
            inflateMenu(attributes.getResourceId(C0489R.styleable.NavigationBarView_menu, 0));
        }
        attributes.recycle();
