...
public final class DebugMetadataKt {
    ...
    public static final StackTraceElement getStackTraceElement(BaseContinuationImpl $this$getStackTraceElementImpl) {
        ...
        checkDebugMetadataVersion(1, debugMetadata.m147v());
        int label = getLabel($this$getStackTraceElementImpl);
        int lineNumber = label < 0 ? -1 : debugMetadata.m143l()[label];
        String moduleName = ModuleNameRetriever.INSTANCE.getModuleName($this$getStackTraceElementImpl);
        String moduleAndClass = moduleName == null ? debugMetadata.m140c() : moduleName + '/' + debugMetadata.m140c();
        return new StackTraceElement(moduleAndClass, debugMetadata.m144m(), debugMetadata.m141f(), lineNumber);
    }
    ...
    public static final String[] getSpilledVariableFieldMapping(BaseContinuationImpl $this$getSpilledVariableFieldMapping) {
        ...
        int[] m142i = debugMetadata.m142i();
        int length = m142i.length;
        for (int i = 0; i < length; i++) {
            int i2 = i;
            int labelOfIndex = m142i[i];
            if (labelOfIndex == label) {
                res.add(debugMetadata.m146s()[i2]);
                res.add(debugMetadata.m145n()[i2]);
            }
        }
