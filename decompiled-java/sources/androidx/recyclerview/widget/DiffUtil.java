...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() { // from class: androidx.recyclerview.widget.DiffUtil.1
        ...
        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f85x - o2.f85x;
            return cmpX == 0 ? o1.f86y - o2.f86y : cmpX;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f85x += range.oldListStart;
                snake.f86y += range.newListStart;
                Range left = rangePool.isEmpty() ? new Range() : rangePool.remove(rangePool.size() - 1);
                left.oldListStart = range.oldListStart;
                left.newListStart = range.newListStart;
                if (snake.reverse) {
                    left.oldListEnd = snake.f85x;
                    left.newListEnd = snake.f86y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f85x - 1;
                    left.newListEnd = snake.f86y;
                } else {
                    left.oldListEnd = snake.f85x;
                    left.newListEnd = snake.f86y - 1;
                }
                stack.add(left);
                if (!snake.reverse) {
                    range.oldListStart = snake.f85x + snake.size;
                    range.newListStart = snake.f86y + snake.size;
                } else if (snake.removal) {
                    range.oldListStart = snake.f85x + snake.size + 1;
                    range.newListStart = snake.f86y + snake.size;
                } else {
                    range.oldListStart = snake.f85x + snake.size;
                    range.newListStart = snake.f86y + snake.size + 1;
                }
                stack.add(range);
            } else {
                rangePool.add(range);
        ...
        while (d <= dLimit) {
            ...
            while (k <= d) {
                ...
                if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || forward[kOffset + k] < backward[kOffset + k]) {
                    ...
                    outSnake.f85x = backward[kOffset + k];
                    outSnake.f86y = outSnake.f85x - k;
            ...
            while (k2 <= d) {
                ...
                if (checkInFwd || k2 + delta < (-d) || k2 + delta > d || forward[kOffset + backwardK] < backward[kOffset + backwardK]) {
                    ...
                    outSnake2.f85x = backward[kOffset + backwardK];
                    outSnake2.f86y = outSnake2.f85x - backwardK;
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f85x != 0 || firstSnake.f86y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f85x + snake.size;
                int endY = snake.f86y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f85x + j;
                    int newItemPos = snake.f86y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = diffResult.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f85x + snakeSize;
                int endY = snake.f86y + snakeSize;
                if (endX < posOld2) {
                ...
                int i = snakeSize - 1;
                while (i >= 0) {
                    if ((diffResult.mOldItemStatuses[snake.f85x + i] & 31) != 2) {
                        z = z2;
                    } else {
                        z = true;
                        batchingCallback.onChanged(snake.f85x + i, 1, diffResult.mCallback.getChangePayload(snake.f85x + i, snake.f86y + i));
                    }
                    i--;
                    z2 = z;
                }
