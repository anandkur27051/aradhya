...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f94mX = new float[numPoints];
        this.f95mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            float distance = (i * pathLength) / (numPoints - 1);
            pathMeasure.getPosTan(distance, position, null);
            this.f94mX[i] = position[0];
            this.f95mY[i] = position[1];
        }
        if (Math.abs(this.f94mX[0]) > 1.0E-5d || Math.abs(this.f95mY[0]) > 1.0E-5d || Math.abs(this.f94mX[numPoints - 1] - 1.0f) > 1.0E-5d || Math.abs(this.f95mY[numPoints - 1] - 1.0f) > 1.0E-5d) {
            throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f94mX[0] + "," + this.f95mY[0] + " end:" + this.f94mX[numPoints - 1] + "," + this.f95mY[numPoints - 1]);
        }
        float prevX = 0.0f;
        ...
        while (i2 < numPoints) {
            ...
            float x = this.f94mX[componentIndex];
            if (x < prevX) {
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f94mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f94mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        }
        float xRange = this.f94mX[endIndex] - this.f94mX[startIndex];
        if (xRange == 0.0f) {
            return this.f95mY[startIndex];
        }
        float tInRange = t - this.f94mX[startIndex];
        float fraction = tInRange / xRange;
        float startY = this.f95mY[startIndex];
        float endY = this.f95mY[endIndex];
