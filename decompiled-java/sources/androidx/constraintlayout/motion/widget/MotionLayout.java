...
public class MotionLayout extends ConstraintLayout implements NestedScrollingParent3 {
    ...
    */
        ...
        if (this.mScene == null || this.mScene.mStateSet == null) {
            ...
            if (tmp_id != -1) {
                ...
                if (stagger != 0.0f) {
                    ...
                    while (i4 < n) {
                        float f2 = f;
                        MotionController f3 = this.mFrameArrayList.get(getChildAt(i4));
                    ...
                    int i5 = 0;
                    while (i5 < n) {
                        MotionController f5 = this.mFrameArrayList.get(getChildAt(i5));
                        float x2 = f5.getFinalX();
                        float y2 = f5.getFinalY();
                        float stagger2 = stagger;
                        f5.mStaggerScale = f4 / (f4 - stagger);
                        f5.mStaggerOffset = stagger2 - ((((x2 + y2) - min) * stagger2) / (max - min));
                        i5++;
                        stagger = stagger2;
                    }
                }
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, C0148R.styleable.MotionLayout);
            int N = a.getIndexCount();
            boolean apply = true;
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == C0148R.styleable.MotionLayout_layoutDescription) {
                    int n = a.getResourceId(attr, -1);
                    this.mScene = new MotionScene(getContext(), this, n);
                } else if (attr == C0148R.styleable.MotionLayout_currentState) {
                    this.mCurrentState = a.getResourceId(attr, -1);
                } else if (attr == C0148R.styleable.MotionLayout_motionProgress) {
                    this.mTransitionGoalPosition = a.getFloat(attr, 0.0f);
                    this.mInTransition = true;
                } else if (attr == C0148R.styleable.MotionLayout_applyMotionScene) {
                    apply = a.getBoolean(attr, apply);
                } else if (attr == C0148R.styleable.MotionLayout_showPaths) {
                    if (this.mDebugPath == 0) {
                        this.mDebugPath = a.getBoolean(attr, false) ? 2 : 0;
                    }
                } else if (attr == C0148R.styleable.MotionLayout_motionDebug) {
