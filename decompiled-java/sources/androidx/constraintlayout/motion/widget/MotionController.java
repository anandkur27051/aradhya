...
public class MotionController {
    ...
    public KeyPositionBase getPositionKeyframe(int layoutWidth, int layoutHeight, float x, float y) {
        ...
        start.left = this.mStartMotionPath.f32x;
        start.top = this.mStartMotionPath.f33y;
        start.right = start.left + this.mStartMotionPath.width;
        start.bottom = start.top + this.mStartMotionPath.height;
        RectF end = new RectF();
        end.left = this.mEndMotionPath.f32x;
        end.top = this.mEndMotionPath.f33y;
    ...
    public float getKeyFrameParameter(int type, float x, float y) {
        float dx = this.mEndMotionPath.f32x - this.mStartMotionPath.f32x;
        float dy = this.mEndMotionPath.f33y - this.mStartMotionPath.f33y;
        float startCenterX = this.mStartMotionPath.f32x + (this.mStartMotionPath.width / 2.0f);
        float startCenterY = this.mStartMotionPath.f33y + (this.mStartMotionPath.height / 2.0f);
    ...
    public void setup(int parentWidth, int parentHeight, float transitionDuration, long currentTime) {
        ...
        if (points[0].mPathMotionArc != Key.UNSET) {
            ...
            while (i10 < size) {
                ...
                values[i10][0] = points[i10].f32x;
                values[i10][r17] = r19[i10].f33y;
                i10++;
                points = points;
                size = size;
            }
    ...
    */
    public boolean interpolate(View child, float global_position, long time, KeyCache keyCache) {
        ...
        float position5;
        View view2 = child;
        float position6 = getAdjustedPosition(global_position, null);
        if (this.mAttributesMap != null) {
            Iterator<SplineSet> it = this.mAttributesMap.values().iterator();
            while (it.hasNext()) {
                it.next().setProperty(view2, position6);
            }
        }
        if (this.mTimeCycleAttributesMap == null) {
            position = position6;
            ...
            while (timeAnimation) {
                ...
                if (aSpline instanceof TimeCycleSplineSet.PathRotate) {
                    ...
                    timeAnimation2 |= aSpline.setProperty(view2, position6, time, keyCache);
                    view2 = child;
                }
            }
            position = position6;
        ...
        if (this.mSpline != null) {
            ...
            if (this.mAttributesMap == null) {
                ...
                for (SplineSet aSpline2 : this.mAttributesMap.values()) {
                    if (aSpline2 instanceof SplineSet.PathRotate) {
                        position5 = position;
                        ((SplineSet.PathRotate) aSpline2).setPathRotate(child, position5, this.mInterpolateVelocity[0], this.mInterpolateVelocity[1]);
                    } else {
                        position5 = position;
                    }
                    position = position5;
            ...
            if (timePathRotate == null) {
                ...
                view = child;
                float position7 = position4;
                position4 = position7;
                timeAnimation |= timePathRotate.setPathRotate(view, keyCache, position7, time, this.mInterpolateVelocity[0], this.mInterpolateVelocity[1]);
            }
            for (int i = 1; i < this.mSpline.length; i++) {
                CurveFit spline = this.mSpline[i];
                spline.getPos(position4, this.mValuesBuff);
            ...
            if (this.mKeyTriggers != null) {
                for (int i2 = 0; i2 < this.mKeyTriggers.length; i2++) {
                    this.mKeyTriggers[i2].conditionallyFire(position4, view);
                }
            }
            position2 = position4;
            c = 1;
            ...
            view = child;
            float position8 = position;
            float float_l = this.mStartMotionPath.f32x + ((this.mEndMotionPath.f32x - this.mStartMotionPath.f32x) * position8);
            float float_t = this.mStartMotionPath.f33y + ((this.mEndMotionPath.f33y - this.mStartMotionPath.f33y) * position8);
            float float_width = this.mStartMotionPath.width + ((this.mEndMotionPath.width - this.mStartMotionPath.width) * position8);
            float float_height = this.mStartMotionPath.height + ((this.mEndMotionPath.height - this.mStartMotionPath.height) * position8);
        ...
        if (this.mCycleMap != null) {
            for (KeyCycleOscillator osc : this.mCycleMap.values()) {
                if (osc instanceof KeyCycleOscillator.PathRotateSet) {
                    position3 = position2;
                    ((KeyCycleOscillator.PathRotateSet) osc).setPathRotate(view, position3, this.mInterpolateVelocity[c2], this.mInterpolateVelocity[c]);
                } else {
                    position3 = position2;
                    osc.setProperty(view, position3);
                }
                position2 = position3;
            }
        }
    ...
    public void getDpDt(float position, float locationX, float locationY, float[] mAnchorDpDt) {
        ...
        float dleft = this.mEndMotionPath.f32x - this.mStartMotionPath.f32x;
        float dTop = this.mEndMotionPath.f33y - this.mStartMotionPath.f33y;
