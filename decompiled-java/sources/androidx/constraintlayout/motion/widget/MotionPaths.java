...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initCartesian(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float startCenterX = startTimePoint.f32x + (startTimePoint.width / 2.0f);
        float startCenterY = startTimePoint.f33y + (startTimePoint.height / 2.0f);
        float endCenterX = endTimePoint.f32x + (endTimePoint.width / 2.0f);
        float endCenterY = endTimePoint.f33y + (endTimePoint.height / 2.0f);
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f32x = (int) ((startTimePoint.f32x + (pathVectorX * position)) - ((scaleX * scaleWidth) / 2.0f));
        this.f33y = (int) ((startTimePoint.f33y + (pathVectorY * position)) - ((scaleY * scaleHeight) / 2.0f));
        ...
        this.mMode = 2;
        this.f32x = (int) (((startTimePoint.f32x + (pathVectorX * dxdx2)) + (pathVectorY * dxdy)) - ((scaleX * scaleWidth) / 2.0f));
        this.f33y = (int) (((startTimePoint.f33y + (pathVectorX * dydx2)) + (pathVectorY * dydy2)) - ((scaleY * scaleHeight) / 2.0f));
    ...
    void initScreen(int parentWidth, int parentHeight, KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        if (!Float.isNaN(c.mPercentX)) {
            ...
            this.f32x = (int) (parentWidth2 * c.mPercentX);
        } else {
            parentWidth2 = parentWidth;
        }
        if (!Float.isNaN(c.mPercentY)) {
            this.f33y = (int) (((int) (parentHeight - this.height)) * c.mPercentY);
        }
        this.mKeyFrameEasing = Easing.getInterpolator(c.mTransitionEasing);
        this.mPathMotionArc = c.mPathMotionArc;
    }
    ...
    void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f32x = (int) ((startTimePoint.f32x + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f));
        this.f33y = (int) ((startTimePoint.f33y + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f));
        this.width = (int) (startTimePoint.width + (scaleX * scaleWidth));
        this.height = (int) (startTimePoint.height + (scaleY * scaleHeight));
    ...
    public void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        ...
        int c2 = c + 1;
        mask[c] = mask[c] | diff(this.f32x, points.f32x) | arcMode;
        int c3 = c2 + 1;
        mask[c2] = mask[c2] | diff(this.f33y, points.f33y) | arcMode;
        int c4 = c3 + 1;
    ...
    public void fillStandard(double[] data, int[] toUse) {
        float[] set = {this.position, this.f32x, this.f33y, this.width, this.height, this.mPathRotate};
        int c = 0;
        for (int i = 0; i < toUse.length; i++) {
